...
class TyreController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('ProductManager::admin.tyre.index',compact('tyre'));
    }
    ...
    public function create()
    {
        ...
        return view('ProductManager::admin.tyre.create',compact('region','country','brand','searchtag','icon','tyrecategory','season'));
    }
    ...
    public function edit(Tyre $tyre)
    {
        ...
        return view('ProductManager::admin.tyre.edit',compact('tyre','region','country','brand','searchtag','icon','tyrecategory','season'));
    }
     ...
     */
    public function size_old(Request $request, Tyre $tyre)
    {
        if($request->method()==='GET'){
            ...
            return view('ProductManager::admin.tyre.size', compact('tyre', 'old_file'));
            //dd($old_file);						
        }
    ...
    public function clone(Request $request, Tyre $tyre)
    {
            ...
            return redirect()->route('ProductManager::admin.tyre.edit', [ 'tyre' =>$new_tyre->id]);
    }
    ...
    public function tyrereorder(Request $request)
    {
        ...
        if($request->method()==='POST'){
            $input = $request->only(['region','country', 'brand', 'searchtag']);
            $input_data=[
                'region'=>$input['region'],
            ...
            $tyres=Tyre::where(['region_id'=>$input['region'], 'brand_id'=>$input['brand'],'search_tag_id'=>$input['searchtag']])->orderBy('order')->pluck('name','id');
        }
        return view('ProductManager::admin.tyre.reorder',compact('region','brand','searchtag', 'input_data', 'tyres'));
    ...
    {
        if($request->method()==='GET'){
            return view('ProductManager::admin.tyre.feature', compact('tyre'));
        }
        if($request->method()=='POST'|| $request->method()=='PUT'){
            $request->validate([
                'title' => ['array'],
    ...
    public function destroy(Tyre $tyre)
    {
        if($tyre->id){
            ...
            $tyre->delete();
            return redirect()->route('ProductManager::admin.tyre.index')->with('status','Tyre deleted');
        }
    }
